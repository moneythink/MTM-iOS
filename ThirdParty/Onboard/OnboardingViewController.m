...
- (void)setSwipingEnabled:(BOOL)swipingEnabled
{
    ...
    _pageVC.dataSource = swipingEnabled ? self : nil;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.fadePageControlOnLastTwoPages) {
        // If the upcoming page is the last object, fade the page control out as we scroll.
        id secondToLast = [self.viewControllers objectAtIndex:[self.viewControllers count]-2];
        
        if (_currentPage == [self.viewControllers lastObject]) {
            _pageControl.alpha = 0.0f;
        }

        else if (_upcomingPage == secondToLast) {
        ...
        else if ((_currentPage == secondToLast) && (_upcomingPage == self.viewControllers[self.viewControllers.count - 3])) {
            _pageControl.alpha = percentComplete;
        }
        
    }
    else if (self.fadePageControlOnLastPage) {
        // If the upcoming page is the last object, fade the page control out as we scroll.
    ...
    if (self.fadePageControlOnFirstPage) {
        // If the upcoming page is the last object, fade the page control out as we scroll.
        if (_upcomingPage == [self.viewControllers firstObject]) {
            _pageControl.alpha = 1.0 - percentComplete;
        }
        
        // Otherwise if we're on the last page and we're moving towards the second-to-last page, fade it back in.
        else if (_currentPage == [self.viewControllers firstObject]) {
            _pageControl.alpha = percentComplete;
        }
    }
}
